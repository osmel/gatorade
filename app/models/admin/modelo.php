...
//----------------**************catalogos-------------------************------------------
 ...
 public function exportar_participantes($data){
          ...
          $this->db->select("p.numero AS numero", FALSE);
          ...
          $this->db->select("count(DISTINCT id_participante, ticket, folio) AS total_tickets", FALSE);
  ...
  /*
          ...
          $this->db->select("sum( ( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '1;', '')) ) /2) *  ((2*(r.responder=1))+ (1*(r.responder<>1)))   )  AS  cant1", FALSE);
          $this->db->select("sum( ( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '2;', '')) ) /2 ) *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant2", FALSE);
          $this->db->select("sum( ( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '3;', '')) ) /2 ) *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant3", FALSE);
          $this->db->select("sum( ( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '4;', '')) ) /2 ) *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant4", FALSE);
          $this->db->select("sum( ( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '5;', '')) ) /2 ) *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant5", FALSE);   
          ...
          $result = $this->db->get();
              ...
              if ( $result->num_rows() > 0 ) {
                        ...
                        foreach ($result->result() as $row) {
                            ...
                            $suma_total = 
                            ((int)$row->cant1)*$this->session->userdata('ip1') +
                            ((int)$row->cant2)*$this->session->userdata('ip2') +
                            ((int)$row->cant3)*$this->session->userdata('ip3') +
                            ((int)$row->cant4)*$this->session->userdata('ip4') +
                            ((int)$row->cant5)*$this->session->userdata('ip5');
 ...
 public function exportar_todo($data){
        ...
        /*
          $this->db->select("( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '1;', '')) ) /2 )  AS  cant1", FALSE);
          $this->db->select("( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '2;', '')) ) /2 )  AS  cant2", FALSE);
          $this->db->select("( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '3;', '')) ) /2 )  AS  cant3", FALSE);
          $this->db->select("( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '4;', '')) ) /2 )  AS  cant4", FALSE);
          $this->db->select("( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '5;', '')) ) /2 )  AS  cant5", FALSE);  

          */        

             $this->db->select("(( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '1;', '')) ) /2 )  *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant1", FALSE);
          $this->db->select("(( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '2;', '')) ) /2 )  *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant2", FALSE);
          $this->db->select("(( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '3;', '')) ) /2 )  *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant3", FALSE);
          $this->db->select("(( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '4;', '')) ) /2 )  *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS  cant4", FALSE);
          $this->db->select("(( (LENGTH(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}')  ) - LENGTH(REPLACE(  AES_DECRYPT(r.tarjeta, '{$this->key_hash}'), '5;', '')) ) /2 )  *  ((2*(r.responder=1))+ (1*(r.responder<>1)))  ) AS cant5", FALSE);
...
public function buscador_participantes($data){
  ...
  public function buscador_detalle_participantes($data){
          ...
          $this->db->select("p.id", FALSE);           
             $this->db->select("( CASE WHEN r.fecha_pc = 0 THEN '' ELSE DATE_FORMAT(FROM_UNIXTIME(r.fecha_pc),'%d-%m-%Y') END ) AS fecha_registro", FALSE);  
              ...
              $suma_detalle = 
                  ...
                  ((int)$row->cant5)*$this->session->userdata('ip5');
                               ...
                               $dato[]= array(
                                      ...
                                      2=>$row->monto,
                                      3=>$row->ticket,
                                      4=>($row->responder ==1) ? 'Si' : 'No',
                                      5=>$row->tiempo, //
                                      6=>$suma_detalle, // total
                                      7=>$row->redes, //
                                      8=>$row->fecha_registro,
                                    );
                      }
